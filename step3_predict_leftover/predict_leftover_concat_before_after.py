...
"""
This approach just squueezes the ViT model and uses it as a feature extractor.
"""
...
def initialize_vanilla_model_optim_loss(flags):  # NOTE: the vanilla version
    ...
    elif flags.img_model == "cnn":  # Feeding concatenated images
        img_model = CNN(cen_mode=False).to(flags.device)
    elif flags.img_model == "cnn-all-parallel":
        img_model = CNN(num_parallel=18).to(
            flags.device
        )  # NOTE: feeding in all channels vanillaly
    else:
